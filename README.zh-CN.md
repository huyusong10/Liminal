...
`任务输入 -> 创建循环 -> 对话生成 Bundle -> READY 方案预览 -> 创建并运行 -> 反馈 -> 下一版 bundle`
...
打开 **创建循环**，选择 **对话生成 Bundle**，然后在对话框里描述需求。Loopora 会调用本机 Agent CLI，并在后续对话中继承 CLI session，把同一份 alignment Skill 指令放进后端 prompt，写入 Agent 返回的 `bundle_yaml`，并在 YAML 通过 bundle 契约校验后显示 READY。

4. 预览、创建并运行

检查生成的任务契约、角色卡片、流程图和源 YAML，然后点击 **创建循环并运行**。Loopora 会把 `spec`、角色定义、workflow 和 loop 作为一组资产物化出来。

5. 可选外部 Agent 路径

如果你更喜欢在 Web 之外做对齐，仍然可以打开 **工具** 页，把 repo-local `loopora-task-alignment` Skill 安装到 Codex、Claude Code 或 OpenCode。这个路径产出的仍是同一种 YAML bundle，可以回到 **创建循环 -> 手动创建循环** 粘贴或导入。
