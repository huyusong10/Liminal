...
```mermaid
flowchart TD
    A["一次 Agent 执行加一次人工 review 够吗？"] -->|够| B["先别用 Loopora"]
    A -->|不够| C["人类会在关键轮次后反复回来判断吗？"]
    C -->|不会| D["直接 Agent 或简单循环通常够用"]
    C -->|会| E["这些未来判断能不能提前塑形？"]
    E -->|不能| F["让人继续留在实时循环里"]
    E -->|能| G["编排 human-shaped Loop"]
```
...
```mermaid
flowchart TD
    J["隐性人类判断"] --> Q["alignment 问题"]
    Q --> W["working agreement"]
    W --> S["spec：什么必须成立"]
    W --> R["roles：谁构建、谁怀疑、谁取证、谁裁决"]
    W --> F["workflow：判断何时发生"]
    S --> X["可运行 Loop"]
    R --> X
    F --> X
...
```mermaid
flowchart LR
    A["描述任务"] --> B["回答 Loop-shaping 问题"]
    B --> C["预览 spec、roles、workflow"]
    C --> D["创建并运行"]
    D --> E["查看证据与裁决"]
```
