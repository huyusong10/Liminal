...
# 场景：长期任务治理闭环从新建到修订再运行

**目标**：确认 Loopora 的主路径不是生成一次 YAML 或跑一次 Agent，而是把任务输入、循环方案、运行证据、GateKeeper 裁决和 harness revision 串成同一个可追踪闭环。
...
7. 从 run 详情进入“基于本次证据修订方案”，确认 revision session 带入 source bundle、run evidence 摘要、GateKeeper verdict 和用户反馈。
8. 对修订后的 READY bundle 做预览，确认它是新的完整方案 revision，而不是只追加一段 prompt 或聊天记录。
9. 用修订后的方案创建并运行新 run，确认新 run 能追溯到上一版方案和本次修订来源。

**预期**：

- 新手主路径保持“任务 / 循环方案 / 证据 / 修订”的心智，不要求先理解内部 YAML。
- READY、run、evidence、GateKeeper 和 revision 都能回到同一份 `spec / roles / workflow / evidence / revision` 契约。
