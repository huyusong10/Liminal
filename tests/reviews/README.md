...
`agent-native-behavior` treats missing phase reports as the primary evidence gap. Its risk hints scan recorded phase reports for shortcut language such as inline, nested, prewritten, or host dispatch; the full Agent adapter design and real-probe handbook remain text context because they intentionally name those shortcuts as prohibited behavior and define the sentinel / dispatch-proof boundary. READY and evidence are core Agent-native anchors and belong in the handbook/artifact reading context.
