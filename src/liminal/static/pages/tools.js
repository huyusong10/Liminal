document.addEventListener("DOMContentLoaded", () => {
  ...
  const WAKE_LOCK_PREF_KEY = "liminal:tools:wake-lock-enabled";
  ...
  function renderRuntimeRuns() {
    ...
    wakeLockRuns.innerHTML = runs.map((run) => {
      const role = run.active_role ? window.LiminalUI.translateRole(run.active_role) : localeText("等待调度", "Waiting");
      const status = window.LiminalUI.translateStatus(run.status || "queued");
