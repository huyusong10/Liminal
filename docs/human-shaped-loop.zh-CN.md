...
普通循环延长的是时间。时间变长后，如果没有治理结构，早期误差会被后续轮次继承、放大、合理化。第一轮如果误解了目标，第二轮可能会沿着误解继续优化；第三轮又会把前两轮的错误当上下文事实；最后得到一个越来越完整、越来越自洽、但方向已经偏掉的结果。
...
如果要提高 Agent 自治程度，真正的问题不是“如何让 Agent 多做几轮”，而是：
...
## 4. Loopora 是 task-scoped judgment compiler
...
## 5. Human-in-the-loop 到 human-shaped loop

传统 human-in-the-loop 的形态是：
...
这当然有效，但它消耗人的连续注意力。人类被迫在任务运行过程中反复上线，重新解释判断标准、指出假完成、要求证据、决定下一步。

Loopora 想做的是一种沟通的时空转换：
...
> 从“人在循环中纠偏”，演进到“人的判断先被编译成循环”。

这不是 human-out-of-the-loop。人类没有消失。人类从“每轮实时纠偏者”变成了“循环设计者”和“证据审计者”。

这也解释了为什么 Loopora 不只是“让 Agent 更努力”。它让 Agent 在一个被人类判断塑形的空间里行动。它可以更自治，但不是无约束自治；它可以跑得更久，但不是盲目跑更久；它可以减少人工参与，但不是把人工判断消失掉。
...
| --- | --- | --- |
| 创意涌现 | 只是发散 20 个点子 | 多轮探索，且每轮要按新颖性、可执行性、风格或反俗套判断转向 |
