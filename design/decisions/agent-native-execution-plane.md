...
- `tests/checks/contracts/test_agent_adapters.py` covers `/loopora-gen -> /loopora-loop`, READY binding, imported-session handoff, `agent_native` state, dispatch proof, no inline submit, control capsules and CLI behavior that does not spawn a nested worker.
