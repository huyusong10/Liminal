...
- run record 的 status 只表示系统生命周期；Loop 是否达标必须通过 task verdict、evidence ledger 和 coverage projection 表达
- run record 创建必须在同一 repository transaction 内拒绝同一 workdir 已存在的 active run；服务层预检查只能作为快速反馈，不能作为唯一并发保护。
...
- repository 读取持久化 JSON 列时必须按列的稳定 shape 降级：对象列损坏或 shape 错误时只返回 `{}`，数组列损坏或 shape 错误时只返回 `[]`，并留下诊断；读取降级不得现场改写 durable row，也不得把数组列伪装成对象列。
- 观察投影读取本地 JSON / JSONL artifacts 时，损坏的单条 JSONL 记录、不可读取文件或非 UTF-8 artifact 只能让对应投影降级为空记录并留下诊断，不得让 run 观察接口整体崩溃。
- `context/latest_state.json` 与 `timeline/stagnation.json` 是可重建的运行上下文 / 进度投影；读取失败时按初始状态重建并覆盖，不能把派生状态损坏升级为 run 契约崩溃。legacy root mirror 只保留兼容观察语义，读失败同样按初始状态重建。
...
这些 legacy mirrors 可以继续被读取，但不再承担真实上下文流转语义。canonical artifact 写入成功后，legacy mirror 写入失败只能留下诊断，不得把本次状态写入整体判为失败。
...
stale/orphan recovery 形成终态时必须复用统一终态 finalization 路径，写入 summary、task verdict artifact、run record 与终态事件；不能只改 run status 而绕过 evidence/task verdict 投影。

workspace safety guard 的基线只追踪用户拥有的工作区文件。`.loopora`、依赖目录、构建输出和常见测试 / 类型 / lint 缓存属于运行或工具生成物，删除它们不得被误判为删除原始用户资产。
基线文件本身属于安全契约；活动 run 检查时如果 baseline 缺失、不可解码或 shape 不符合预期，必须失败关闭，不得把缺失 baseline 当作“无原始文件”而跳过保护。
...
- run 注册在 durable record 创建前写出的临时 run 目录，如果最终未能入库，必须 best-effort 清理；清理失败按 cleanup diagnostics 记录，不能遮蔽原始注册失败。
