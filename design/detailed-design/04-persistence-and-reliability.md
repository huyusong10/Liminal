...
全局状态目录默认放在 `~/.loopora/`，但实现必须允许通过环境变量覆盖到其他目录，以便测试隔离、受限执行环境和多实例并行时仍能保持同一持久化语义。品牌更名后的兼容层仍需接受旧的 `LIMINAL_HOME`，并在已有 `~/.liminal/` 状态存在时保持可读写，直到调用方完成迁移。
...
Loopora 允许改动用户项目，但不允许默认走“破坏式重建”。
