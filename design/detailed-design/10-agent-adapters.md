...
- 宿主入口和 CLI 的人类可读输出应把结果说成 Loop 预览与 preview URL；当预览已 READY 时，还必须给出同一宿主 Agent 的下一步：先审查 preview URL，确认符合任务判断后再在同一 Agent 会话运行 `/loopora-loop`，不得把用户默认引向 Web 启动 run。`READY`、`candidate`、bundle hash 和候选字节数是 Core/JSON/诊断事实，不是用户默认要阅读的成功主语。
- 受管 `/loopora-gen` slash command / skill / command 的菜单级描述和文件首屏必须表达“编译任务判断”，至少点出任务目标、伪完成风险和必需证据；不能只说“从当前任务生成预览”。受管 `/loopora-loop` 的菜单级描述必须表达它启动的是已审查且保留任务判断与证据要求的 Loop 预览，而不是普通继续执行命令。
...
- 候选方案文件校验失败的人类可读输出必须给出修复操作面：原始候选文件、session 预览副本、按错误归纳的 repair focus，以及“修文件 -> 重新 `/loopora-gen` -> READY 后 `/loopora-loop`”的顺序。JSON/API 仍以结构化字段表达状态，终端文案不把 raw validation error 当成唯一恢复路径。
- 宿主入口提交了候选方案文件但 Core 校验失败时，返回的 preview URL 必须打开 Web 修复面，而不是只打开一条失败对话：如果当前 `artifacts/bundle.yml` 仍可读取，Web 应展示它的治理投影和源文件入口；如果读取失败，Web 应展示源文件错误与重新同步入口；两种情况都不得显示创建运行动作，直到重新校验为 READY。Web repair 文案仍必须保持 Agent-first 恢复链路：修源候选文件 -> 重新 `/loopora-gen` -> READY 后回到同一宿主 Agent `/loopora-loop`，不能把“创建并运行”作为 Agent-first 候选修复后的下一步。
...
- 宿主入口若能取得 session / thread identity，Core binding、`agent_candidate_received` event、READY 后的 `agent_candidate_ready_content` event 和 Web `agent_entry_launch` / `agent_entry_start` 投影必须保留同一 `host_context_id`。Web 里的 CLI fallback 与 run detail 复制命令必须带 `--context-id <host_context_id>`，避免用户从 Web 复制命令后退回 workdir binding、找不到刚审查的预览，或在同一 workdir 多个候选之间拿错 Loop。若宿主没有稳定 identity，字段为空，命令继续按 workdir binding 退化。
...
- 若宿主入口无法可靠取得完整会话上下文或没有提交候选方案文件，adapter 只能退化为打开 Web Loop setup / review 预填入口；该入口保留用户消息与来源 provenance，并在 transcript 中可见说明当前只是 Web review、尚非可运行 Loop，以及需要补齐成功标准、假完成、证据预期、执行策略、判断取舍、残余风险和本地治理责任；CLI 人类可读输出也必须先说明“尚不可运行 / 没有候选方案文件”或 not-fit 原因，再列出同一组 review focus 和下一步，而不是只打印 URL；Web session API 还必须投影 `agent_entry_review`，让页面在 transcript 前显示 `/loopora-gen Web review` 桥接卡、不可运行状态、缺少候选方案文件 / not-fit 原因、宿主任务锚点、候选方案文件状态、推荐审查回复、可点击分支和下一步判断清单，避免用户把普通聊天 transcript 误认为已经生成可运行 Loop，也避免用户对着内部缺项重新造 prompt；推荐审查分支写入 transcript 时必须显式携带宿主任务锚点，供后续 working agreement 和 invocation prompt 白盒追踪；若宿主消息已经表达一次性任务、直接回答、不需要 Loop 或后续不会产生新证据，transcript、binding / JSON 与 CLI 输出必须优先解释 not-fit 与需要重新定义后续证据 / handoff / GateKeeper 裁决，不能只给通用补齐清单；默认 transcript 和托管入口说明应说“候选方案文件 / candidate plan file”，不把 YAML 当作用户默认主语；不得自动启动后端 alignment Agent，也不得伪装成 READY 或可运行 Loop。JSON/API 字段可继续使用 `requires_web_alignment` 表达内部状态；`loopora_fit_contradiction` 只表达当前无候选 fallback 的 not-fit 诊断，原始信号由 `agent_candidate_received` event 保留用于排障。
...
- Agent-first `/loopora-gen` 的 READY 输出不能只是 URL 和下一步命令。CLI JSON 与人类可读输出都应携带同源的 `ready_review_projection`，覆盖 Loopora fit、成功面、假完成风险、证据偏好、执行策略、判断取舍、残余风险、coverage target 数量、traceability 映射、GateKeeper 收口和诊断计数；人类可读输出应把这些投影列在 `ready_review` 下，并要求用户确认这些判断后再 `/loopora-loop`。
- 这些未 ready / 需修复状态是可恢复流程，不是内部异常展示面：人类模式应只输出恢复入口并以非零退出阻止运行，不再追加结构化错误日志或 raw conflict 文案；`--json` 模式应输出同义的 structured recovery payload（含 `ready=false`、`loop_recovery` kind、binding/session、preview URL）并以非零退出，供托管 `/loopora-loop` 入口稳定解析。托管 `/loopora-loop` skill / command 必须读取该 payload 并停止角色派发：`finish_web_review` 指回 Web review，`repair_candidate_plan_file` 指回源候选文件 / 预览副本 / validation repair focus，`preview_not_ready` 指回 `/loopora-gen` 或 Web review；不得把这些状态折叠成“先运行 /loopora-gen”，也不得隐式重建方案文件。
...
- 重复调用时必须分开看 run lifecycle 与 task verdict：非终态 active run 继续当前 step，不会被重复创建；terminal run 只有在 task verdict 为 `passed` 或 `passed_with_residual_risk` 时回放完成状态。若 terminal run 的 task verdict 是 `failed`、`insufficient_evidence`、`not_evaluated` 或缺失，用户再次主动调用 `/loopora-loop` 表达继续取证意图，Core 应基于同一个已导入 Loop 创建新的 agent-native run、更新 session / binding 的 `linked_run_id`，并把上一轮 run id、task verdict、coverage 缺口、top gaps 与下一步聚焦项写入新 run 的 continuation context、run contract、StepContextPacket、capsule、CLI 输出和 Web 当前交接投影；不能让用户卡在“生命周期已完成”的回放里，也不能开始一轮不知道上一轮为什么证据不足的空白运行。
- 已由 Agent-first `/loopora-gen` 导入或启动的 Loop 保留 Agent-first 启动边界：Loop 详情、run 详情和 Web start API 不能把“开始新运行 / 重新运行”降级成 headless worker；它们必须投影回同一宿主 Agent 的 `/loopora-loop` 命令，并在直接 API 调用时以 409 返回相同结构化投影。Web 仍可观察证据、运行状态、裁决和 artifact，但不能成为这个 Loop 的执行宿主。
- Agent-first Web 的 `/loopora-loop` 引导在上一轮 terminal task verdict 未通过时，不能只显示复制命令和一段泛化说明。`agent_entry_start` 投影必须带出上一轮 run id、task verdict、covered / missing check count、missing ids、top gaps、next focus，以及 task verdict / coverage artifact path；Loop 详情和 run 详情都应在命令旁展示“下一轮证据接力”，让用户在点击续跑前看到下一轮为什么要补这些缺口。Loop 详情作为长期回访面，还必须在同一块交接卡里提供复制续跑命令、打开证据运行、用证据改进 Loop 的动作，避免用户从 Loop 工作台先迷路到运行详情才能进入下一轮判断。这个投影不是新事实源，只复用 terminal run artifact 与 continuation context 规则。
...
- Agent-native step claim 必须同时落盘完整 step capsule 与 result wrapper template：`capsule.json` 是宿主 Agent / Web / 人工审查共同读取的完整交接契约，包含 prompt、output schema、role dispatch、judgment contract、evidence rules 与 context refs；`.loopora/agent_outbox/{adapter}/<run>__<step>.result.template.json` 是可填写的提交壳，预填 `loopora_host_dispatch`，并携带提交时会被忽略的 `loopora_result_contract` 辅助块，用于白盒展示当前 step、行动权限、证据缺口、known evidence ids、evidence ref 规则与 output schema。`result` 不能是空壳，必须由 Core 按 output schema 预铺 schema-shaped scaffold，所有字符串、数字、布尔、枚举和数组 item 示例使用 `null` 这类无效占位；目标 role agent 提交前必须替换这些占位、把无内容数组改成 `[]`，并删除不用提交的可选占位字段。未替换 scaffold 的模板必须被 output schema 校验拒绝。CLI、Web 当前 handoff 面和托管 `/loopora-loop` 入口必须指向同一个 result template / outbox / submit hint，并直接显示 result_file_contract、known evidence id 数量和“替换 null 占位、保留 host dispatch”的填写规则；托管入口应先读取模板、按 `loopora_result_contract` 只填 `result`、保留模板供审计，再用 `submit_hint.command` 提交，不能鼓励宿主 Agent 凭记忆手写 wrapper。
...
- 当 `submit` 或 `loop/next` 到达终态且 task verdict 不是 `passed` / `passed_with_residual_risk` 时，JSON 与人类可读输出都必须显式给出 `task_next_action`：说明 run lifecycle 已完成但任务未被证明，提示在同一宿主 Agent 会话再次运行 `/loopora-loop` 以开启带上一轮证据缺口的下一轮，并指出若 Loop 本身需要调整则从 run URL 的 evidence improvement 入口进入 Web 对话。人类可读输出不得为非通过裁决打印未限定的 `agent_native: complete`；只能表达为 lifecycle closed / task unproven，并保留具体 task verdict。不能只打印 `complete: true` 或 `succeeded` 后让用户自行推断是否收尾。托管 `/loopora-loop` 入口使用 `--json`，因此必须读取 `task_next_action.kind=continue_evidence`，报告 verdict 与下一步命令，而不是把 `complete: true` 当成任务完成。
- 当 `loop/next/submit` 返回下一步 capsule 时，人类可读输出必须同时给出可执行交接摘要：`next_target_agent`、action policy、required coverage 已覆盖 / 缺口计数、top coverage gaps、context path 和 submit hint。完整 prompt、output schema 与 evidence rules 保留在 JSON/capsule 和 context artifact 中；终端默认面只回答“下一步交给谁、按什么边界做、缺什么证据、从哪里读、如何交回”。
- `submit` 因 wrapper、dispatch proof、output schema、evidence refs、coverage target 或 read-only artifact 声称被拒绝时，人类可读错误必须保留失败关闭语义，同时给出修复面：待修 result 文件、仍然打开的 active step、目标 Agent、context path、可归纳的 schema / evidence / target 修复焦点，以及重新查看 active `next --json` 契约再提交的命令。失败请求仍不得写入 canonical output、handoff 或 evidence ledger。
- `loopora agent <adapter> loop/next/submit --json` 也必须返回同一语义的 `judgment_contract` 与 `task_next_action` 投影；managed entry 不能只依赖非 JSON 终端文案来知道被冻结的判断输入或非通过终态的继续动作。
...
- Web 与 CLI 的安装成功反馈必须把实际目标目录和下一步用户动作投影出来：回到对应 Coding Agent，先用任务目标、伪完成风险和必需证据喂给 `/loopora-gen`，审查 READY Loop 预览，再在同一 Agent 会话运行 `/loopora-loop` 启动或继续任务。JSON 输出仍只承载结构化状态，不把提示文案变成接口契约。
- Web Tools 在安装成功或刷新发现已安装入口时，必须留下持久交接卡：显示 adapter、目标项目、运行 `/loopora-gen` 前要带入的任务判断清单、可复制的 `/loopora-gen` / `/loopora-loop` 顺序，以及由 adapter service 返回的 managed entry 文件状态与 manifest 路径。文件清单是白盒诊断证据，不得取代 slash command 下一步。
- CLI 人类可读安装反馈应把“Coding Agent 入口已安装、目标项目、下一步动作”放在文件清单之前；下一步必须先提示回到对应 Agent 并带入任务目标、伪完成风险和必需证据，再审查 Loop 预览并在同一 Agent 会话运行 `/loopora-loop`。managed files、manifest、adapter status 等 ownership 细节只作为诊断信息，不能盖过 `/loopora-gen -> Loop preview -> /loopora-loop` 的默认路径。
- 当安装发现同名入口文件或宿主配置不是 Loopora-managed，CLI 和 Web 都必须把失败表达成“入口未安装且项目文件保持不变”，列出冲突文件或配置，并给出检查、移动 / 重命名、换目标项目目录、重新安装的恢复顺序；不得只暴露 raw ownership error 或让用户以为需要强制覆盖。
...
- Codex / Claude Code / OpenCode 的 Agent-host real probe 目标互不共享 workdir、Loopora home、Web port 和 binding state；发布验证入口可以把每个 host 目标拆成独立 pytest 子进程并行运行，并在等待期间输出 heartbeat 与可追踪日志路径。real Agent-host harness 还应在 workdir 下写 `.loopora/real-probes/real-agent-phase-report.json`，把 candidate、validation、binding、runtime activity、run events、agent-native state、coverage、task verdict、role output 与 sentinel log 压成诊断投影。默认发布路径不得在 host command template 中主动传 `--model`、`--effort`、`--variant` 或 `model_reasoning_effort`，应委托真实宿主使用当前默认配置；若要验证外部配置覆盖，必须通过显式 override 开关让本次 real probe 失去默认宿主配置证明含义。并行、heartbeat 与 phase report 只属于 harness 调度 / 观察优化，不得削弱 sentinel、entry provenance、subagent dispatch proof 或终态断言。
