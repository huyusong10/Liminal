...
`用户描述任务 -> Web 方案对话 -> 后端 Agent CLI -> 方案文件（内部 bundle YAML）-> READY Loop 预览 -> 运行前复核 -> 创建 Loop 并运行`

稳定承诺：

- 用户可以完全不理解 prompt 拼装、YAML 或外部 Agent 操作。
- 用户也不需要先理解 `bundle / spec / roles / workflow`，但 READY 预览必须让这些治理 surface 可检查。
- Web 路径里的显式确认对象是可见 working agreement；READY Loop 是经过校验的预览，后续语义是 review / create / run，不引入独立的 confirmed Loop 状态。Web-only READY 预览可以直接创建运行，但必须先显式确认已复核判断投影、证据收口和运行目录，避免把硬校验误读成人工已审查。
- 对话问题必须优先围绕能改变 Loop 形状的 task-scoped judgment：Loopora fit、成功面、假完成、证据偏好、执行策略、残余风险策略、判断取舍、本地治理责任、角色姿态、workflow 形状和信息流。
- 最终产物仍然必须是单文件 YAML bundle；这是候选 Loop 的交换格式。
- READY 只表示 bundle 文件已经存在并通过硬校验，不表示 loop 已经导入或运行成功。
- READY 预览必须读取当前 canonical `artifacts/bundle.yml` 并返回本次读取的校验结果；历史 session validation 只是上次硬校验记录，不能让被人工或宿主改坏的当前文件继续显示为可审查、可导入或可运行。
- READY 预览必须同时提供运行出口和审查后调整出口。用户在 READY 后继续输入修改意见时，系统应把它作为对当前候选 Loop 的 review feedback，保留已确认工作协议和当前 bundle 作为上下文，重新编译整份候选方案；不能把它退回普通 clarifying 状态后禁止输出 bundle，也不能静默导入旧预览。Agent-first READY 预览进入调整时，默认草稿也必须保留“审查后回到同一 Agent 运行 `/loopora-loop`”的交接语义，避免调整动作把用户带回 Web-run 心智。review feedback 默认保留当前候选与 working agreement 的语言，除非用户明确要求翻译，避免一次跨语言反馈破坏已确认判断的 traceability。
- READY 在默认界面中的主语义是“Loop 已准备好，先复核再运行”；`bundle`、`import` 和 YAML 只在专家源文件操作或错误调试中出现。
...
- Agent-first READY 预览是审查和交接面，不是 Web-run 面：页面主操作必须指向 `/loopora-loop`，并明确保留宿主 Agent-native 执行路径；点击 Web 主操作不得调用 alignment import/run 直接启动 headless executor。Web-only 对话编排路径必须显示复核门槛后再创建运行；手动导入路径仍可显示按预览导入或创建运行。
- Agent-first Loop 一旦由 `/loopora-loop` 导入或运行，后续 Loop 详情和 run 详情仍必须保留同一执行平面；Web 可以展示当前 run、证据和 `/loopora-loop` 命令，并必须提供复制该命令的主动作，但不得通过“开始新运行 / 重新运行”把同一个 Loop 切回 headless。若宿主 binding 有 session / thread identity，Web 展示和复制的 CLI fallback 必须保留 `--context-id`，让用户从 Web 复制命令也回到同一个候选 / run 绑定。若当前绑定 run 已终态但 task verdict 未通过，Web 必须把下一步表达为“回到宿主 Agent 用 `/loopora-loop` 开启下一轮补证据”，并在结构化 projection 中暴露该 next action，避免用户把终态生命周期误读为已证明。
- Agent-first entry 无候选方案文件时，Web 对话页不能只把事实藏在 transcript 里。session API 只有在当前 session 仍需要 Web review、尚未形成 working agreement，且尚未 READY / imported / running 时才投影 `agent_entry_review`，页面必须在 transcript 前展示 Web review 桥接状态：来源是 `/loopora-gen`，当前尚不可运行，原因是缺少候选方案文件或 Loopora fit 存疑，并直接展示宿主任务锚点、候选方案文件是否缺失、推荐审查回复和可点击分支。推荐分支必须把任务锚点显式带回 transcript，使后续 working agreement 能被这次 `/loopora-gen` 的任务判断牵引，而不是让用户对着八个内部缺项重新造 prompt。桥接卡仍要列出补齐完成标准、伪完成风险、证据预期、Loopora fit、执行策略、判断取舍、残余风险和本地治理责任的下一步。一旦 review 进入正常 working agreement 确认，就应收起 bridge，把主流程交给确认选项；已经提供候选 YAML 的修复失败、READY 或已运行 session 不得继续暴露 `missing_candidate_plan` 投影，避免白盒 API 把已存在的方案文件说成缺失。
...
| `/loops/new/bundle` | 顶层“编排”的默认对话编排工作台；READY 预览与复核后创建运行 |
...
- READY 预览、加载失败和源文件错误态必须保持可读横向排版。标题、状态、主操作和运行前最重要的风险摘要不能因为 flex/grid 收缩变成逐字竖排；不可执行的主操作必须隐藏或禁用。风险摘要应优先独占一行，证据 / 判断 / 目录这类事实 chip 可以换行，不能反过来挤压风险文本。
...
从 run evidence 发起的改进 source context 必须包含 run status、run contract / evidence artifact paths、冻结 judgment contract 摘要、evidence coverage summary、最终 task verdict projection、recent evidence summary 与 raw GateKeeper verdict；coverage summary 不能只保留一句原因或 top gaps，必须把 ledger / coverage path、evidence count、required check covered / missing counts、covered / missing check ids、target bucket counts、top gaps、evidence kind counts、artifact ref count、residual risk count、risk signals 和 latest GateKeeper 投影一起带入上下文，确保下一版 Loop 能被具体缺口牵引。recent evidence summary 应保留可用 artifact refs。改进 Agent 应以 task verdict 作为用户可见结论，以 raw GateKeeper verdict 作为解释材料，并以 frozen judgment contract 作为原始人类判断基线；不能只看 raw verdict 而忽略 run lifecycle、coverage gate、residual-risk bucket 或运行前已冻结的判断标准后得到的最终裁决。

从 run evidence 跳转到 Web 对话页时，这些 source context 不能只藏在 prompt 或 `working_agreement.source` JSON 里。页面必须在 transcript 前展示上一轮证据桥：来源 run、run lifecycle、task verdict、缺失检查数、weak / blocking target 数、top gaps、missing check ids、risk signals、recent evidence 摘要和 evidence artifact paths。这个桥接卡的作用是让用户在 Agent 回复前就看到“下一版 Loop 为什么应该先补这些缺口”，并能用白盒文件追查来源；它不是新的事实源，所有内容都来自已脱敏的 source context。
...
| spec file | spec 路径、受限 raw spec 内容或摘要；若文件不能按 UTF-8 读取，降级为可读 source 错误并保留路径，而不是阻断 session 创建；同时说明该 spec 只是起点而不是完整 workflow |

source context 不能注入 `.loopora/invocations`、stdout/stderr、完整事件日志、token、认证头、Cookie 或其他调试材料。若同一目录存在多个候选来源，页面必须让用户选择其一；未选择时不能悄悄挑选最新产物启动。workdir context 选项列表可以有数量上限，但“重新生成 / 忽略旧产物”必须始终保留；来源过多时宁可裁剪旧来源，也不能裁掉用户显式放弃旧上下文的出口。历史 alignment session 只有在 `artifacts/validation.json` 或 DB validation 为 ok、当前 `artifacts/bundle.yml` 仍可通过结构与语义校验，并且 bundle `loop.workdir` 仍匹配当前 workdir 时，才可作为 workdir 上下文候选；旧 validation 不能让已被改坏或指向其他目录的文件继续成为可改进来源。
...
- 每次 Agent 调用必须写入新的 `invocations/<n>/` 目录，不能覆盖旧 prompt / output / stdout / stderr；READY 后审查再调整、自动修复和后续对话都必须留下可对比的 invocation 记录。`artifacts/validation.json` 仍是当前 bundle 校验事实源，同时对应 invocation 下也保存本轮 `validation.json` 便于白盒审计。
...
- 后端维护独立于运行状态的 alignment stage：`clarifying -> agreement_ready -> confirmed -> compiling -> ready`；READY 后用户继续给出审查意见时进入 `ready_review -> compiling -> ready`。Agent 可以提交阶段候选，但不能自己把 session 推进到 confirmed。确认前的 not-fit / blocked 输出若带有可展示说明，仍应停在 `waiting_user` 对话态，让用户补充反复判断、新证据或假完成风险，而不是把“可能不需要 Loopora”当成系统失败。
- Web-only 新建 alignment session 的前端首条消息必须来自结构化 judgment brief：任务目标、伪完成风险和必需证据是必填判断面，并以清晰 section 进入 transcript 与后续 prompt；补充说明可以追加阻断风险、残余风险或优先级，但不能代替这三项。选择已有 session 继续时可发送普通回复；从新 workdir 重新生成时仍需补齐 judgment brief，避免 Web 路径退化成只输入任务标题。
- `failed` 若来自已落盘候选方案文件的校验失败，Web 必须把它表达为可修复的方案文件状态：展示当前可读取的 Loop 预览或源文件错误、保留打开源文件与重新同步入口，并隐藏创建运行动作，直到当前 canonical `artifacts/bundle.yml` 重新通过校验成为 READY。
- 可修复失败态必须先给用户下一步，而不是先铺开 raw 校验错误：页面应展示修复焦点、源文件路径、重新同步入口和“通过校验前不可运行”的状态；原始校验错误只作为折叠诊断保留。这个失败态不是 run failure，也不是 READY preview。
...
- Agent 主导对话语义：用户可以乱序描述、改目标、问产品问题或给出部分答案；Agent 负责理解语境、提出下一步、收敛 working agreement 或说明 blocked。
- 后端主导阶段承认：Agent 可以提交 `clarifying / agreement / bundle / blocked` 候选，但服务层只在候选满足当前 compiler gate、readiness evidence、traceability 与 bundle 校验后推进状态。
- 主动注入与被动检测共同组成 compiler：prompt 注入当前 gate 和产品约束；检测阻断跳阶段、空泛证据、未投影判断、断链 workflow、语言错位、workdir fact 编造和不合约 YAML。
- 检测结果应区分可自动修复与必须追问用户：YAML shape、handoff、evidence query、GateKeeper fan-in 等结构缺口可回灌给 Agent 修复；Loopora fit、假完成、证据偏好、执行策略、残余风险、判断取舍和本地治理责任不清时必须回到对话。
...
- 结构化输出包含 `decision_options`；当 literal boolean `needs_user_input=true` 且本轮需要用户选择时，Agent 应提供 2-4 个可点击选项，每个包含稳定 id、用户可读 label / description、recommended 标记和点击后追加到 transcript 的 `user_reply`。它只属于编译期对话面，不进入 bundle、working agreement 或 run evidence。服务层在记录 assistant transcript 时保留这些选项，前端只允许用户点击最新一条 assistant 选项；点击效果等同于发送一条普通用户消息。字符串、数字等非布尔 `needs_user_input` 不能触发用户输入门禁或澄清问题改写；blocked phase / status 仍可在有可展示 assistant message 时把 session 留在等待用户状态。
- 服务层只展示清洗后仍有 2-4 个有效项且至少一个 boolean `recommended: true` 的 Agent 选项；有效项必须同时具备非空 label、description 和 user_reply。不足、过窄、缺少描述、缺少推荐项，或用字符串 / 数字等非布尔值伪装推荐标记时，必须回退到服务层默认任务风险选项，避免把单项确认或无推荐问卷伪装成可选取舍。
...
- Agent 在展示 working agreement 或输出 bundle 前，应私下做 agreement-to-bundle traceability checklist：每个已确认判断项必须落到 `collaboration_summary`、`spec.markdown` / `Role Notes`、role prompt / posture、`workflow.collaboration_intent`、step `inputs`、workflow controls 或 GateKeeper evidence 规则；任务范围、成功面、假完成、证据偏好、执行策略、残余风险策略、判断取舍和本地治理责任都不能只停留在 readiness evidence。只存在于 `agreement_summary`、readiness evidence、transcript、metadata / loop 名称或隐性推理里的判断，不能作为 READY 前的已编译判断。服务层在 READY 校验时也要用已确认 working agreement 做粗颗粒 traceability 与类别语义检查，阻断把 refund、audit、notification、permission、owner / follow-up、AGENTS.md、design 或其他任务特异判断留在确认协议里、最终却输出通用 bundle 的情况。
- 这道 traceability 检查必须支持用户语言中的任务特异词；中文 working agreement 中反复出现的高信号对象（例如退款、授权、审计、支付、客服交接）不能因为没有英文 token 而绕过编译边界。
- readiness evidence 必须覆盖 Loopora fit、任务范围、成功面、假完成、证据偏好、执行策略、残余风险策略、判断取舍、本地治理责任、角色姿态、workflow 形状和 workdir 事实 / 假设，并且在用户确认前显式说明最终证据会如何投影到 Proven / Weak / Unproven / Blocking / Residual risk（中文可用等价语义）。这些字段是后端门槛，但门槛只检查字段存在、非占位、证据桶投影、task-scoped 反模式、open questions、workdir fact grounding、明显不可管理的残余风险策略、显式 Loopora fit 矛盾（例如证据承认一次 Agent + review 或单轮执行已足够、benchmark/test-harness-only / 稳定 benchmark / proof harness / 测试套件已足够、后续不会产生新证据或判断不需要活过本次聊天却仍要求生成 bundle），以及 `It works well`、`avoid bugs`、`need proof`、`use three roles` 这类明显项目管理占位话术；具体文字是否充分、是否用了某些关键词、是否以固定句式说明 Loopora fit，不应由正则硬阻断。readiness evidence 的质量主要由 prompt、可见确认摘要和用户确认来治理。`open_questions` 只能为空、无遗留问题或仅等待确认，不能把仍会改变成功面、证据、残余风险、角色或 workflow 的问题藏进 ready agreement。
- 若 Workdir Snapshot 显示 `AGENTS.md`、适用于所选子目录的父级 `AGENTS.md`、`design/README.md`、`design/` 或 `tests/`，Agent 不能声称其内容已被观察，除非 snapshot 或对话真的提供了内容；但这些 marker 应被视为项目本地治理入口，并在相关 bundle 中转成角色责任或验证期望。后端 traceability 校验不能只接受最终 bundle 复述 marker 名称；当 confirmed working agreement 把这些 marker 作为治理事实时，最终 bundle 必须同时表达 Builder 读取、Inspector / Custom 验证和 GateKeeper 阻断 / 弱证据处理责任。
...
- READY 预览中的 control summary / judgment map 必须把成功面、假完成风险、证据偏好、执行策略、残余风险策略、判断取舍和本地治理责任作为独立投影项展示，并给出来自 `spec.markdown` / `Role Notes`、角色 posture / prompt 或 `workflow.collaboration_intent` 的证据片段；role posture 映射必须来自角色责任、姿态或 prompt 内容，不能只靠角色名、archetype 或编号；其中残余风险策略只有命名可接受风险及其负责人 / 跟踪 / 后续 / 接受路径，或明确失败关闭时才算已映射，空泛风险话术只能暴露为 warning 和 traceability 缺口；本地治理责任卡片只展示已形成 Builder 读取、Inspector / Custom 验证、GateKeeper 阻断 / 弱证据处理完整运行责任链的证据，summary-only 或 partial 信号只能暴露为 traceability 缺口 / diagnostics；只有通用协作摘要、主要风险摘要、证据标题、workdir marker 名称或元数据名称不能证明这些判断已经编译进可运行 surface。
...
- READY 校验必须使用与普通 Loop 创建相同的 loop executor 语义；未知 executor kind / mode、custom preset mode、缺少 command 参数或非法 reasoning effort 不能先进入 READY 再在导入阶段失败。角色未显式提供 executor 字段时继承 normalized loop executor defaults，避免 READY 预览显示 Claude / OpenCode / custom Loop，执行时角色却回到 Codex 默认。
- Web alignment 生成的 bundle 必须使用 `loop.completion_mode: "gatekeeper"`，让 READY 后的默认运行路径以 evidence-backed GateKeeper verdict 作为 Loop 裁决，而不是只用 rounds 生命周期收束。专家手动编排 / 导入路径可以继续使用受支持的 `rounds`，但未知、布尔或数字 `completion_mode` 必须失败关闭，不能绕过 GateKeeper 收束校验。
...
- 控制摘要、traceability 摘要与 bundle diagnostics 必须由后端 bundle projection 生成，不能由前端临时猜测；它们不是新的事实源。关键信息条只展示少量状态，判断地图只展示 judgment -> surface 的压缩映射和非阻断风险提示，不复制完整 `spec / roles / workflow` 内容。READY 页面和手动导入预览必须把 `collaboration_summary`、`spec.markdown#Task`、`role_definitions[].posture_notes`、`workflow.steps[].inputs` 这类内部 surface id 翻译成“治理摘要 / 任务契约 / 角色姿态 / 步骤交接输入”等用户语义；原始锚点只留在 API、YAML、专家源文件或调试材料中。
- Web-only READY 预览的直接创建运行动作必须经过运行前复核门槛：页面显示由同一份后端 projection 得出的证据路径、判断投影和收口方式摘要，并在用户确认已复核判断地图、证据路径和运行目录前禁用创建运行按钮。该确认只是当前页面上的 review acknowledgement，不是新的持久 confirmed Loop 状态，也不能替代后续 run evidence / GateKeeper 裁决。
- 判断地图的 `mapped` 标记必须按 literal boolean 处理；前端只能把 `mapped: true` 展示为已投影，字符串或其它 truthy 值不能把缺口显示成已映射。
- GateKeeper 门禁摘要的 `enabled` 标记必须按 literal boolean 处理；前端和二次治理摘要只能把 `enabled: true` 展示为存在证据收口 GateKeeper，字符串或其它 truthy 值不能把 rounds / 未配置方案显示成 GateKeeper 裁决。
...
- 流程图标签必须留在 SVG 可视边界内，并在常见手机宽度下保持非零可见尺寸；不得靠外层横向溢出、裁剪或整页滚动来隐藏标签问题。
...
- 若用户确认运行前复核后选择“创建 Loop 并运行”，立即创建 run 并跳转到 run 详情
- 若只导入，则跳转到 loop 或 bundle 详情

稳定规则：

- 导入后的资产仍属于同一 bundle 生命周期。
- Web-only 对话编排的立即运行按钮在复核确认前必须禁用；若前端被程序化触发，也必须在调用 import/run 前检查复核状态并把焦点带回复核门槛。
...
- `GET /api/alignments/sessions/{id}` 返回的 `working_agreement` 可以包含 `readiness_checklist` 与 `readiness_evidence`；旧 session 没有 evidence 或缺少新增 evidence 维度时仍可读取，但新 Web alignment 生成前必须具备完整 checklist / evidence，包括 Loopora fit、执行策略、残余风险策略、判断取舍和本地治理责任。
...
- 从 run evidence 创建改进 session 时，source context 是 best-effort 摘要；损坏或不可读的 evidence artifact 只能降级为空摘要，不得阻断 session 创建。只要路径仍可定位，source context 应继续暴露 task verdict、ledger、coverage 与 manifest 的 artifact path，便于用户或 Agent 追查损坏材料。
...
- `/api/bundles/preview` 不创建 bundle、loop 或 run；它只复用 bundle 契约校验和预览投影。响应必须保留与 READY 预览同源的 `control_summary`，并在顶层提供 `traceability` 与 `diagnostics` 别名，让 Web 导入面、Agent-first 审查面和机器调用方看到同一份 judgment projection，而不是各自猜测 bundle 是否真正外化了判断。
...
- Tools 页不提供 alignment Skill 安装；外部 YAML 只能作为专家导入 / 导出格式进入同一校验、预览和运行路径。
