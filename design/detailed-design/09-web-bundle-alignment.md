...
| 路由 | 职责 |
|------|------|
| `/loops/new` | 兼容旧入口的轻量选择页，解释 Bundle-first 与手动专家模式的区别 |
| `/loops/new/bundle` | 顶层“新建任务”的默认入口；对话生成 bundle、READY 方案预览与创建运行 |
...
- 顶层“新建任务”能直接进入 Bundle 对话页；旧创建入口仍能兼容进入 Bundle 对话页；Bundle 对话页能从“输入需求”启动 alignment session。
