...
source context 不能注入 `.loopora/invocations`、stdout/stderr、完整事件日志、token、认证头、Cookie 或其他调试材料。若同一目录存在多个候选来源，页面必须让用户选择其一；未选择时不能悄悄挑选最新产物启动。workdir context 选项列表可以有数量上限，但“重新生成 / 忽略旧产物”必须始终保留；来源过多时宁可裁剪旧来源，也不能裁掉用户显式放弃旧上下文的出口。
...
- 检测结果应区分可自动修复与必须追问用户：YAML shape、handoff、evidence query、GateKeeper fan-in 等结构缺口可回灌给 Agent 修复；Loopora fit、假完成、证据偏好、残余风险和判断取舍不清时必须回到对话。
...
- readiness evidence 必须覆盖 Loopora fit、任务范围、成功面、假完成、证据偏好、残余风险策略、判断取舍、角色姿态、workflow 形状和 workdir 事实 / 假设，并且在用户确认前显式说明最终证据会如何投影到 Proven / Weak / Unproven / Blocking / Residual risk（中文可用等价语义）。这些字段是后端门槛，但门槛只检查字段存在、非占位、证据桶投影、task-scoped 反模式、open questions、workdir fact grounding，以及显式 Loopora fit 矛盾（例如证据承认一次 Agent + review 或单轮执行已足够、benchmark-only / 稳定 benchmark 已足够、后续不会产生新证据或判断不需要活过本次聊天却仍要求生成 bundle）；具体文字是否充分、是否用了某些关键词、是否以固定句式说明 Loopora fit，不应由正则硬阻断。readiness evidence 的质量主要由 prompt、可见确认摘要和用户确认来治理。`open_questions` 只能为空、无遗留问题或仅等待确认，不能把仍会改变成功面、证据、残余风险、角色或 workflow 的问题藏进 ready agreement。
- 若 Workdir Snapshot 显示 `AGENTS.md`、`design/README.md`、`design/` 或 `tests/`，Agent 不能声称其内容已被观察，除非 snapshot 或对话真的提供了内容；但这些 marker 应被视为项目本地治理入口，并在相关 bundle 中转成角色责任或验证期望。后端 traceability 校验不能只接受最终 bundle 复述 marker 名称；当 confirmed working agreement 把这些 marker 作为治理事实时，最终 bundle 必须同时表达 Builder 读取、Inspector / Custom 验证和 GateKeeper 阻断 / 弱证据处理责任。
