...
| Web UI | 编排编辑页可以把步骤主区做成摘要卡片，并用点击卡片切换当前角色；只要被选中的角色快照、步骤顺序与 workflow snapshot 保持一致即可 |
| Web UI | 编排编辑页可以通过单个设置浮窗只编辑 step 级覆盖项，并把绑定的角色快照作为只读信息展示；只要提交前后的 workflow / prompt 语义不变即可 |
