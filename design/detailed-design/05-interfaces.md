...
Web UI 的顶层信息架构按用户任务组织，而不是按内部对象模块组织：

| 顶层入口 | 稳定目的 | 目标路由 |
|----------|----------|----------|
| 工作台 / Workbench | 查看当前任务、已创建 loop 与最近运行状态 | `/` |
| 新建任务 / New Task | 通过对话生成循环方案并创建运行 | `/loops/new/bundle` |
| 方案库 / Plans | 管理可复用 bundle / 循环方案 | `/bundles` |
...
| 暴露 guided entry 的物化结果 | Web 内置 alignment 与外部 AI Agent + Skill 都必须产出同一类 bundle；Web / CLI / API 对 bundle 生命周期动作保持一致 |
...
| 范围 | 允许差异 |
|------|----------|
| Web UI | 提供可视化 workflow 编辑器、角色选择器、教程入口与实时页面导航；顶层导航可以按用户任务收敛为工作台、新建任务与方案库，并把资源管理、专家创建、工具、教程和显示设置放入次级菜单，只要底层路由和对象能力保持可达 |
| CLI | 提供同步等待与后台执行开关 |
...
| Web UI | 内置预设编排列表卡片可以只保留“适用场景”等摘要，不必在卡片正文直接铺开 spec 内容；对应详情页应通过单个只读入口按钮打开 spec 样例弹窗，并收敛为单个预览窗口：内置预设显示“真实需求 + 为什么适合该流程 + 示例 spec”的 scenario example，自定义编排显示按当前 workflow 生成的模板预览 |
...
| Web UI | 教程页可以使用独立的阅读壳层，但应保持极简：优先讲清为什么要用 loop、如何用决策板选择流程、以及去哪一页操作；教程应以 5 条核心流程的真实样例优先，而不是规则优先，不必展开冗长的 context 文件流转说明；这块选择器应优先作为页面内原生布局组件出现，解释文案直接写进组件内部，不再在图旁并排堆说明文字；它可以采用决策板、匹配表或其他高可读形式，不要求装饰性的连线或伪流程图，但必须先回答“该不该用 Loopora”，再回答“最先缺什么，对应哪条流程”；教程文案需要明确强调：Loopora 的价值不是证明AI Agent 不行，而是减少人类反复回来确认、裁决和纠偏的次数；教程里的“查看流程样例”应直接以只读弹窗在当前页打开，而不是把用户跳转到编排页；README 可以引用由这块同源组件生成的稳定截图，而不是维护另一份独立示意图 |
...
| 文件预览 | 按不可信内容处理，不能直接执行；结构化文本（如 JSON / JSONL）预览在遇到坏内容时应尽量保留可读部分，并把解析异常作为预览元数据返回，而不是把整份内容直接降级成 5xx |
| 输入规范化 | 非法、缺失或越界输入在接口层被拦截或标准化；JSON 写接口只接受对象 body，坏 JSON 或非对象 body 必须以明确的 4xx 错误返回，而不是把解析异常泄漏成 5xx |
