...
| `alignment session` | 通过对话编排或调整 Loop | 调用本机 AI Agent CLI，在 bundle 通过校验后提供 READY 预览与复核后创建运行 |
...
Tools 页的 Agent 接入交接不只是命令备忘录。它必须在用户复制 `/loopora-gen` 前提醒宿主 Agent 需要拿到任务目标、伪完成风险和必需证据，并把后续路径表达为“带着这些判断运行 `/loopora-gen` -> 审查 READY Loop 预览 -> 同一 Agent 会话运行 `/loopora-loop`”。Web 在这条路径中是证据、缺口和裁决观察面，不是默认执行宿主。

CLI 顶层帮助页、`loopora init --help` 和各 Agent 安装子命令帮助都属于 first-use surface：即使保留专家命令，帮助描述必须先说明 `loopora init <agent>` -> 带着任务目标、伪完成风险和必需证据运行 `/loopora-gen` -> 审查 Loop 预览 -> 同一 Agent 会话 `/loopora-loop` 的默认路径，并让安装入口与本地 Web 入口在子命令列表中先于资源库、角色、流程、spec 和从现有 spec 直接运行的专家入口出现。
...
创建方式选择页属于默认路径：推荐项应表达为用自然语言整理任务判断和证据要求，而不是通用聊天；对话编排空态必须提示用户输入任务、假完成风险、必需证据和阻断风险，不得退回“你想让 Loopora 做什么”这类普通助手输入框心智。对话编排 composer 的首个输入面必须把任务目标、伪完成风险和必需证据拆成可填写的 judgment brief，并把这三项写入首条 transcript / alignment prompt；自由文本只作为补充上下文，不能是 Web-only 新建 Loop 唯一承载判断的地方。手动分支应表达为用户已明确契约、检视角色和运行流程，而不是要求新手理解底层运行资产。
...
| 编排工作台 | 对话编排 Loop、运行目录、READY 预览、复核后创建并运行、导入方案文件、手动编排 | 源 YAML 和底层对象只在专家视图或二级入口中出现 |
...
- 当已有 Loop 来自 Agent-first `/loopora-gen` / `/loopora-loop` 绑定时，Loop 工作台、Loop 详情和 run 详情必须把 Web 定位为观察 / 管理证据面，而不是新的执行宿主；启动或再次运行入口应显示回到 `/loopora-loop` 的 Agent-native 指引，并把该命令做成可复制主动作，HTTP start / rerun 调用必须拒绝静默创建 headless run。若绑定 run 已终止但 Loop 裁决仍未通过或证据不足，Web projection 应说明 `/loopora-loop` 会在同一宿主 Agent 中开启下一轮取证，而不是把 lifecycle `succeeded` 暗示成任务已完成；Loop 详情还应直接给出打开上一轮证据运行与基于证据改进 Loop 的动作。
...
- `READY` 可以作为内部状态或小状态标识；Web-only 编排的默认主动作必须表达为“Loop 已准备好，先复核再运行”，并在确认判断投影、证据路径和运行目录前禁用立即运行；Agent-first 预览或修复分支必须把 READY 后的动作指回同一宿主 Agent `/loopora-loop`。
...
- run detail 的 progress 阶段、服务端 seed 与终态 chip 也属于 lifecycle 投影；终点只能表达运行收束 / 结束，不能使用 `Done`、`完成` 这类会被读成任务达标的文案。若终态 task verdict 是 `insufficient_evidence`、`failed`、`not_evaluated` 或带残余风险通过，progress 终点与 live progress 卡片必须显式投影 Loop 裁决语义和非绿色视觉语气，不能因为 run lifecycle 是 `succeeded` 就显示成无条件完成。
...
- Loop 工作台的活动区、已保存 Loop 卡片和 Loop 详情运行历史都可以用运行状态胶囊表达生命周期，但同一行或同一卡片遇到终态 task verdict 时必须同时展示 Loop 裁决，例如证据不足、未通过、带残余风险通过或已通过，避免用户在进入详情前把 `succeeded` 看成任务已证明。
- Loop 工作台卡片和 Loop 详情的一句话摘要属于首屏判断面；当最近运行已经终态且 task verdict 不是缺失中的进行态时，摘要必须优先复述 Loop 裁决和证据结论，不能直接展示 Agent 的自然语言 summary 作为主摘要。尤其在 `insufficient_evidence`、`failed` 或 `not_evaluated` 时，Agent summary 里的“完成 / all done”只能留在追查材料，不能在概览卡片里盖过证据裁决。
...
- `loopora agent <adapter> gen` 的人类可读输出应把主对象说成 Loop 预览和 preview URL；当预览已 READY 时，输出必须把下一步指回同一宿主 Agent 的 `/loopora-loop`，并同时投影本次预览的运行前审查摘要：Loopora fit、成功面、假完成风险、证据偏好、执行策略、判断取舍、残余风险、coverage target 数量、traceability 映射和 GateKeeper 收口方式。这样用户即使留在 slash command 里，也能先看到这份 Loop 凭什么值得运行，避免把 Web preview 误当成 Agent-first run 的执行宿主或把 READY 当成完成判断；当它为 preview 或 run URL 自动启动或复用本地 Web 服务时，必须同时显示 Web 状态，避免用户拿到 localhost 链接却不知道服务是否已经可用；`READY`、`candidate` 和 bundle hash 只作为 JSON 字段或诊断事实保留，不能成为默认成功提示的主文案。
- Agent-first runtime 的同步输出必须保留冻结判断契约的可追溯入口；当 run artifacts 可用时，人类可读 `loop/next/submit` 输出应打印 `run_contract_path`、source plan provenance、判断摘要、Loopora fit、执行策略、判断取舍、本地治理责任与角色姿态，JSON 输出应返回同义的 `judgment_contract` 投影，并保留 `source_bundle` provenance、check mode、completion mode、workflow preset 与 coverage target trace 这类执行锚点；在终态完成时继续分开报告 `run_status` 与 `task_verdict`。若 task verdict 不是 `passed` / `passed_with_residual_risk`，人类可读输出不能打印未限定的 `agent_native: complete`，只能表达 lifecycle closed / task unproven 并给出下一轮取证动作。宿主 Agent 不能只看 `succeeded` 或 `complete` 就判断 Loop 已被证据证明。
- Agent-first step capsule、result template、CLI 输出和 Web handoff 中的复制命令必须保留入口 provenance：当 run 由项目级 `/loopora-gen` / `/loopora-loop` 进入时，`submit_hint.command` 必须包含 `LOOPORA_AGENT_ENTRY_SOURCE=<source>` 和 `--entry-source <source>`，并由 Agent-native state 继承到后续 step。若宿主 session / thread identity 可用，Web handoff 与 run detail 的 `/loopora-loop` fallback 也必须带 `--context-id`，让复制命令回到同一宿主绑定而不是 workdir fallback。用户复制 Web 或 CLI 里的提交命令时，不应因为使用了方便入口而丢失这次运行来自哪个宿主入口的证据。
- Agent-first result template 不能只是空 `result` 壳。模板必须在顶层附带 `loopora_result_contract` 辅助块，说明该块提交时会被忽略，并投影当前 step id、role、action policy、required coverage、known evidence ids、evidence ref contract、evidence rules 与 output schema，让宿主 Agent 打开模板即可知道该填哪些字段、可引用哪些证据和缺口是什么。`result` 本身必须按 output schema 预铺可编辑 scaffold，用 `null` 作为无效占位来提示必填形状；未替换占位的模板提交必须失败关闭，避免空壳或自然语言总结被误当作结构化证据。该辅助块不得包含完整 prompt、完整 judgment contract 或未脱敏 role output；Web 首屏仍只显示模板路径和摘要，不内联 output schema。
- `/loopora-loop` 托管入口必须把 result template 当成提交的白盒工作面：宿主 Agent 应读取 `next_step.submit_hint.result_template_*`，按 `loopora_result_contract` 填写 `result`，保留 `loopora_host_dispatch` 与 provenance，并用 `submit_hint.command` 提交。CLI 与 Web 当前交接面必须直接投影模板填写规则、known evidence id 闭集数量和“替换 `null` 占位后再提交”的恢复规则，不能只给路径让宿主或用户猜。入口说明不得让宿主 Agent 从终端示例凭记忆重写 wrapper，因为这会绕开当前 step 的 evidence ref 闭集、coverage target 与 output schema。
- Agent-first 候选校验失败时，人类可读 CLI 输出必须把失败表达成“方案文件需要修复”，并提供可操作修复入口：原始候选文件路径、session 中的预览副本路径、由校验错误归纳出的修复焦点，以及修复后重新 `/loopora-gen`、READY 后再 `/loopora-loop` 的下一步。Web preview 的修复面也必须使用同一条恢复链路，不显示“创建并运行”作为下一步。原始 `validation_error` 可以保留作诊断事实，但不能成为唯一用户指引。
- Agent-first `/loopora-loop` 在当前 preview 尚未 ready 时必须保持拒绝运行，但人类可读 CLI 输出不能只打印领域错误。它应重新投影当前 binding/session 的恢复入口：Web review URL 与 review focus，或 plan file / preview copy / repair focus / rerun `/loopora-gen` 步骤；若需要 Web URL，应与 `/loopora-gen` 一样自动启动或复用本地 Web 服务。这样用户误跑 `/loopora-loop` 时会回到同一条审查 / 修复链路，而不是丢失刚才的上下文。
- Agent-first Web 预览页在 session 已进入 `running_loop` 且绑定了 run 时，必须保留同一份预览与 Agent-native 指引，主动作应打开当前 run，不能清空预览区或重新显示“创建并运行”。页面应同时显示 `/loopora-loop`、CLI fallback、workdir 和当前 run 链接，让用户知道 Web 是观察证据面，继续执行仍回到宿主 Agent。
- Agent-first `submit` 校验失败时，人类可读 CLI 输出必须把失败表达成“result JSON 需要修复且当前 step 未推进”，并提供 result 文件、active step、目标 Agent、context path、schema / evidence / coverage target 修复焦点和重新查看 `next --json` 契约的命令；该 schema lookup 命令同样必须保留 `LOOPORA_AGENT_ENTRY_SOURCE` 和 `--entry-source` provenance。即使 result 文件在进入 Core 前就因 JSON 语法、wrapper 形状或非对象 payload 失败，也必须进入同一修复路径，而不是只打印 parser 错误。原始 schema path、parser 或 dispatch 错误可以保留作诊断事实，但不能成为唯一恢复路径。
...
| `/loops/new/bundle` | 编排工作台 | 默认通过对话编排 Loop、READY 预览、复核后创建并运行；共享侧栏保留导入方案文件与手动编排入口 |
...
Loop 工作台为空时可以提供两个轻量 first-run 出口：安装 Coding Agent 入口，以及进入 Web 对话编排。空态必须在出口前提示用户先准备任务目标、伪完成风险和必需证据，避免首页成为只复制命令或只点创建按钮的任务标题入口。它们只服务“还没有任何 Loop 时如何开始”的空态，不把工作台重新变成创建表单或专家资源入口。
...
- `current_agent_step` 属于活动 Agent-native run 的当前交接投影：只展示 step id、role、目标宿主 Agent、action policy、required coverage 摘要、top gaps、context path、capsule path、result template、result outbox、submit hint，以及从上一轮非通过 task verdict 派生的 continuation 摘要；不得内联完整 prompt、output schema 或未脱敏 role output。continuation 只投影上一轮 run id / path、task verdict 状态与摘要、coverage 计数、missing ids、top gaps 和下一步聚焦项，让 Web 用户不打开 raw JSON 也能看出本轮为什么继续取证。它来自 run 目录下的 Agent-native state 文件，缺失或损坏时退化为空对象，不影响 run 生命周期事实。
...
- run detail 的 SSE `run_finished` 事件若携带 `task_verdict_status`、source 或 summary，前端必须先把这些裁决字段合并进当前 run 投影，再刷新 progress / highlight / console meta；实时观察路径不能等下一次 snapshot 才知道任务是否证据不足、未通过或带风险通过。若历史或异常终态事件缺少 `task_verdict_status`，Web 观察投影必须按 `not_evaluated` 和 warning 语气展示，不能回退成 lifecycle `succeeded` 的绿色完成。
...
- run detail 首屏的冻结判断契约只展示足以解释本次裁决的紧凑摘要，例如来源方案、Loopora fit、成功面、假完成、证据偏好和残余风险；完整 run contract、长路径、完整 collaboration summary 和 workflow intent 属于追查入口，不能把首屏证据面挤成契约全文阅读页。
- run detail 观察到 `awaiting_agent` / active Agent-native step 时，必须在首屏证据区直接展示当前 Agent 交接：目标 Agent、上下文文件、提交命令、行动权限和主要证据缺口。用户不应为了知道下一步由谁执行、缺什么证据或如何提交而先打开 raw state JSON。
- run detail 必须从结果页直接暴露当前 Loop 的导出入口，让用户在查看 Loop 裁决后可以把本次判断结构带走、复用或丢弃，而不需要先跳转到专家 bundle 列表。
- run detail 的结果记录动作只记录用户把当前证据裁决写入审计事件，不改变 run status、task verdict、bundle lineage 或 Loop 定义；按钮与事件投影必须按 task verdict 区分为“记录通过结论 / 记录带风险通过 / 记录未证明结论 / 记录失败结论 / 记录未评估结论”等，不能把 `insufficient_evidence`、`failed` 或 `not_evaluated` 表达成接受任务完成、普通证据结论或绕过证据不足。`insufficient_evidence` / `failed` / `not_evaluated` 终态的主行动应优先指向用证据改进 Loop 或继续取证；非 Agent-first 的再次运行入口也应被标记为下一轮取证，而不是泛化的普通重跑。记录动作只是审计出口，不是新的自动演化状态。同一 evidence source event 与同一 task verdict status 的记录动作必须幂等：首次记录写入审计事件，之后页面显示对应 verdict 的已记录状态，重复提交也只能复用原事件，不能刷出多条等价记录。记录事件必须带上当时可用的 task verdict、`recorded_verdict_kind`、coverage、manifest artifact path、证据桶计数、evidence source event id、source bundle provenance，以及完整 normalized `judgment_contract`（含 source bundle provenance、check mode、completion mode、workflow preset 与 coverage targets），让后续审计能知道用户记录的是哪份证据裁决和哪套冻结判断契约；若证据摘要因 artifact 损坏不可读取，仍必须保留同一 payload shape，并显式标记 evidence unavailable，不能退化成只有事件 id 的不可审计记录。
...
- Tools 页面可以显示只读健康摘要、计数和问题明细，并可提供定位目录、打开上级目录或复制路径等人工排查动作；不得提供删除按钮、自动修复按钮或把诊断结果变成主工作流阻塞条件。由于 Tools 也是 Agent-first 首次安装入口，本地资产问题明细属于维护面板，必须默认收起，只保留计数和显式查看入口，避免把历史残留误读为 `/loopora-gen -> /loopora-loop` 的阻塞项。
