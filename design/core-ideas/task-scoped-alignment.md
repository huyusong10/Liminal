...
Alignment 在交付 working agreement 或 bundle 前，还应能私下完成一张 agreement-to-bundle traceability checklist：每个已确认的判断项都必须至少落到一个可运行或可读 surface，例如 `collaboration_summary`、`spec.markdown`、role prompt / posture、`workflow.collaboration_intent`、step `inputs` 或 GateKeeper evidence 规则。只停留在 `agreement_summary`、readiness evidence、聊天记录或隐性推理里的判断，不算已经编译进 Loop。
...
- 不问抽象人格问题，优先问能改变 Loop 的场景选择题：两个都不完美的结果里，用户会拒绝哪一个，为什么？
