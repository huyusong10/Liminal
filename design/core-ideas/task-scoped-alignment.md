...
本文档定义对话式 alignment 如何帮助用户编排 Loop。当前实现里的 Web 问答是内置 alignment compiler；Agent-first `/loopora-gen` 是另一个 first-class 候选入口，但它不改变本文件描述的 Web 对话阶段门槛。
...
`运行 Loop -> 自动迭代并收集证据 -> 输出运行状态、Loop 裁决与结果`
...
- 不把“用户确认了”当作充分条件；确认必须足以说明成功面、假完成、证据偏好、执行策略、角色姿态和 workflow 形状。
...
与它并列的其他取得 Loop 场景可以继续表达为：

- `Coding Agent 当前任务 -> /loopora-gen -> Core 校验 / READY 预览 -> /loopora-loop`
...
| `spec` | task contract、成功面、假完成、证据偏好、执行策略、判断取舍与残余风险政策 |
| `role definitions` | 各角色的 task-scoped posture、证据责任与项目本地治理责任 |
| `workflow / orchestration` | 角色顺序、handoff、证据 fan-in、纠偏入口、本地治理 checkpoint、自动迭代和收束条件 |
...
- 启动 run
- 在 Agent-first 路径中分发 execution capsule 并接收宿主 Agent 的结构化回填
- 在显式 headless 路径中调用外部 AI Agent CLI
- 按 Loop 推进状态、证据、handoff 与裁决
- 记录事件、产物、证据和终态
...
Web alignment Agent 来自 Loopora 内置 session。Agent-first adapter 可以让当前 Coding Agent 生成候选 bundle，但它必须通过同一 Core 校验、READY 预览、bundle 导入和 run/evidence 边界；宿主 Agent 不能自己宣布 READY、导入资产或裁决 Loop。用户仍可手动导入 YAML，但通用外部 alignment Skill 不作为 Web compiler 的替代品。Loopora 本体不决定用户应该如何演进 Loop；它只负责提供取得 / 调整 Loop 的入口、导入导出、运行和证据记录。
...
- Agent-first 候选入口绕过 Core 校验、READY 预览或 bundle 导入。
