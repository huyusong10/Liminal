...
3. 判定权不属于生成器。  
   `Generator` 负责改动；`Tester` 负责取证；`Verifier` 负责裁决。
...
6. `Challenger` 不是常驻角色。  
   只有停滞或回退时才介入；默认循环是 `Generator -> Tester -> Verifier`。

7. 接口层必须同构。  
   Web、CLI、API 表达的是同一个 loop 定义，不允许长期存在一边能做、另一边不能做的核心能力差异。

8. 系统首先是本地编排器，不是云端任务平台。  
...
- `Tester` / `Verifier` / `Challenger` 默认不应修改源文件。
- `Verifier` 的通过结论必须可映射到 checks、metrics 或约束。
...
| Generator 自己宣布“已经完成” | 违反“判定权不属于生成器” | 通过条件退化成自我报告 |
| Tester 为了让测试通过而直接改代码 | 违反“角色职责分离” | 证据与修复混杂，责任边界消失 |
