...
| 层级 | 说明 | 示例 |
|------|------|------|
| 主工作流 | Loopora 必须稳定支持的任务生命周期 | 编排 Loop、运行 Loop、自动迭代、收集证据、运行状态、任务裁决 |
...
- 新用户只需要理解：任务、Loop、运行、证据、运行状态、任务裁决。
...
- README、教程和 Web 首屏必须继续围绕“任务 / Loop / 运行 / 证据 / 运行状态 / 任务裁决”，而不是把内部字段当作入口词。
...
一个 Loop 必须同时表达三类运行输入，并在 run 中生成证据输出；用户最终看到的结果必须同时区分“系统是否跑完”和“证据是否证明任务达标”：
...
| 结果投影 | 任务裁决 | 说明证据是否证明任务达标、未达标、证据不足或存在残余风险 |

前三者定义 Loop 如何运行；`evidence` 让用户复盘一次 run 是否可信。运行状态不能替代任务裁决：例如 `rounds` 模式跑满轮数只说明 run 正常收束，不等于任务被证明完成。
...
6. 我查看白盒证据、运行状态和任务裁决结果。
7. 如果我不满意，可以手动改 Loop、导入新 YAML，或再次通过对话让 Agent 生成候选改进；这些都是编排场景，不是 run 的默认阶段。

`bundle` 是专家资产和内部交换单元；默认界面应优先使用“任务 / Loop / 运行 / 证据 / 运行状态 / 任务裁决”。
